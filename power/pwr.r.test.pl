...
my %opts;
GetOptions (\%opts,"r=s",  "n=s", "alternative=s", "sig=s", "power=s",  "output=s", "html=s",
);
...
library(pwr)
res= pwr.r.test(r=$opts{r},n=$opts{n},alternative="$opts{alternative}",sig.level=$opts{sig},power=$opts{power} )
sink("results")
res
sink(file=NULL)
