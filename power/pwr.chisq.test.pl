...
my %opts;
GetOptions (\%opts,"w=s",  "N=s", "df=s", "sig=s", "power=s",  "output=s", "html=s",
);
...
library(pwr)
res= pwr.chisq.test(w=$opts{w},N=$opts{N},df=$opts{df},sig.level=$opts{sig},power=$opts{power} )
sink("results")
res
sink(file=NULL)
...
library(pwr)
w = sqrt( $numerator )
res= pwr.chisq.test(w=w,N=$opts{N},df=$opts{df},sig.level=$opts{sig},power=$opts{power} )
sink("results")
res
sink(file=NULL)
