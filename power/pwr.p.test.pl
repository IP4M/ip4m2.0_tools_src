...
my %opts;
GetOptions (\%opts,"h=s",  "n=s", "alternative=s", "sig=s", "power=s",  "output=s", "p1=s", "p2=s", "html=s",
);
...
library(pwr)
res= pwr.p.test(h=$opts{h},n=$opts{n},alternative="$opts{alternative}",sig.level=$opts{sig},power=$opts{power} )
sink("results")
res
sink(file=NULL)
...
res= pwr.p.test(h=h,n=$opts{n},alternative="$opts{alternative}",sig.level=$opts{sig},power=$opts{power} )
sink("results")
res
sink(file=NULL)
