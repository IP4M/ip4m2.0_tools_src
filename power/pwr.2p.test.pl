...
my %opts;
GetOptions (\%opts,"h=s",  "n=s", "sig=s", "power=s", "alternative=s", "output=s", "p1=s", "p2=s", "html=s",
);
...
library(pwr)
res= pwr.2p.test(h=$opts{h},n=$opts{n},sig.level=$opts{sig},power=$opts{power},alternative="$opts{alternative}")
sink("results")
res
sink(file=NULL)
...
res= pwr.2p.test(h=h,n=$opts{n},sig.level=$opts{sig},power=$opts{power},alternative="$opts{alternative}")
sink("results")
res
sink(file=NULL)
