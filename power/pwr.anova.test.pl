...
my %opts;
GetOptions (\%opts,"k=s",  "n=s", "f=s", "sig=s", "power=s",  "output=s", "N1=s", "u=s", "var=s",  "html=s",
);
...
library(pwr)
res= pwr.anova.test(k=$opts{k},n=$opts{n},f=$opts{f},sig.level=$opts{sig},power=$opts{power} )
sink("results")
res
sink(file=NULL)
...
my @units = @ARGV;
my @formulas;
for(my $i = 0; $i <= $#units; $i+=2){
    my $ni = $units[$i];
    my $ui = $units[$i+1];
    push @formulas, "($ni/$opts{N1})*($ui - $opts{u})^2";
}
my $numerator = join " + ", @formulas;

print RCMD <<"RSCRIPT";

library(pwr)
f = sqrt( ( $numerator )/$opts{var} )
res= pwr.anova.test(k=$opts{k},n=$opts{n},f=f,sig.level=$opts{sig},power=$opts{power} )
sink("results")
res
sink(file=NULL)
