...
my %opts;
GetOptions (\%opts,"u=s",  "v=s", "f2=s", "sig=s", "power=s",  "output=s", "R2=s", "R2a=s", "R2ab=s", "html=s",
);
...
library(pwr)
res= pwr.f2.test(u=$opts{u},v=$opts{v},f2=$opts{f2},sig.level=$opts{sig},power=$opts{power} )
sink("results")
res
sink(file=NULL)
...
library(pwr)
f2 = $opts{R2}/(1-$opts{R2})
res= pwr.f2.test(u=$opts{u},v=$opts{v},f2=f2,sig.level=$opts{sig},power=$opts{power} )
sink("results")
res
sink(file=NULL)
...
library(pwr)
f2 = ($opts{R2ab}-$opts{R2a})/(1-$opts{R2ab})
res= pwr.f2.test(u=$opts{u},v=$opts{v},f2=f2,sig.level=$opts{sig},power=$opts{power} )
sink("results")
res
sink(file=NULL)
