...
my %opts;
GetOptions (\%opts,"d=s",  "n1=s", "n2=s", "sig=s", "power=s", "alternative=s", "output=s", "u1=s", "u2=s", "var=s", "html=s",
);
...
library(pwr)
res= pwr.t2n.test(d=$opts{d},n1=$opts{n1},n2=$opts{n2},sig.level=$opts{sig},power=$opts{power},alternative="$opts{alternative}")
sink("results")
res
sink(file=NULL)
...
res= pwr.t2n.test(d=d,n1=$opts{n1},n2=$opts{n2},sig.level=$opts{sig},power=$opts{power},alternative="$opts{alternative}")
sink("results")
res
sink(file=NULL)
