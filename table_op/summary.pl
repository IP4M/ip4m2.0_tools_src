...
library('pastecs')
data = read.table("__matrix__", header=T, com='', quote="",sep="\t", row.names=1, check.names=F)
data = t(data)
summary = stat.desc(data, basic=TRUE, desc=TRUE, norm=TRUE, p=0.95)
summary = t(summary)
write.table(summary,"out.table",quote=FALSE ,sep=\"\\t\")
...
get_html_link_v2($html, "Peak Table Basic Statistics Summary Results", "$output", "tabH");
# getRTable(in, out)
sub getRTable{
    my $in = shift;
    my $out = shift;
