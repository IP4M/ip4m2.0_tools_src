...
#plot_data
head(plot_data)
ggplot(plot_data, aes(x=Var2, y=value)) + geom_boxplot() + ylab("log2(v+1)") + xlab("") + ggtitle("Boxplot ( analize v >= $min )")+ theme(panel.background = element_rect(fill = NA), panel.grid.major = element_line(colour = NA ), panel.grid.minor = element_line(colour = NA), panel.border = element_rect( colour = "gray30", fill = NA, size=1.5 ) )
