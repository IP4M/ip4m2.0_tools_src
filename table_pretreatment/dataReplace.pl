...
my %opts;
GetOptions (\%opts,"i=s","o=s","m=s","html=s", "a=s", "k=s", "val=s");
...
print RCMD <<"RSCRIPT";
data = as.matrix(data)
imData = impute::impute.knn(data, $k)
data = imData[[1]]
RSCRIPT
}
...
print RCMD <<"RSCRIPT";
data = as.matrix(data)
imData = imputeLCMD::impute.QRILC(data)
data = imData[[1]]
RSCRIPT
}
...
write.table(data,"out.table",quote=FALSE ,sep=\"\\t\")
...
get_html_link_v2($html, "Zero Filling Results", "$output", "tabH");
