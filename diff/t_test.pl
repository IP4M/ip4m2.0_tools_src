...
my %opts;
GetOptions (\%opts,"i=s","o=s","g=s","html=s", "corrected=s", "paired=s", "var_equal=s", "thrN=s", "o2=s",
);
...
my $paired = $opts{paired};
my $var_equal = $opts{var_equal};
...
check_data_v3($input, $group_file, $paired);
...
for (i in 1 : nrow(data)) {
    ...
    okk\$p[i] = tryCatch(
    {
        tets = t.test(x , y, alternative = "two.sided", paired = $paired, var.equal=$var_equal)
        tets\$p.value
    }, error = function(e){
        1
    }
...
get_html_link_v2($html, "T-test Results", "$output", "tabH", "$output_sign", "tabH");
