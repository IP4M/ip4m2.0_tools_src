...
my %opts;
GetOptions (\%opts,"i=s","o=s","g=s","html=s", "corrected=s", "paired=s", "tf=s", "thrN=s", "o2=s",
);
...
for (i in 1 : nrow(data)) {
    ...
    okk\$p[i] = tryCatch(
    {
        tets = wilcox.test(x , y, alternative = "two.sided", paired = $paired, correct=$tf)
        tets\$p.value
    }, error = function(e){
        1
    }
...
get_html_link_v2($html, "Wilcoxon-test Results", "$output", "tabH", "$output_sign", "tabH");
